...
When this flag and `FF_KANIKO_PATH_SCOPED_REGISTRY_AUTH` are enabled, kaniko preflights the authorization scopes required for cross-repository mounts using the destination credential. If those scopes cannot be authorized, it falls back to the normal blob upload path instead.
Defaults to `false`.
Becomes default in `v1.29.0`.
